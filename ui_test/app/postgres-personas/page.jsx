...
import { deletePostgresPersona, getPostgresPersonas, insertPostgresPersona, updatePostgresPersona } from "../actions/postgresService";
...
const PostgresPersonasPage = () => {
  ...
  const reloadData = async () => {
    ...
    try {
      const result = await getPostgresPersonas();
      setPersonas(result || []);
    } catch (loadError) {
  ...
  const savePersona = async (persona) => {
    ...
    try {
      if (modalMode === "create") {
        await insertPostgresPersona(persona);
        await showSuccess(t("common:msgInsertedTitle"), t("common:msgInsertedText"));
      } else {
        await updatePostgresPersona(persona);
  ...
  const confirmDelete = async (persona) => {
    ...
    try {
      await deletePostgresPersona(persona.idPersona);
  ...
  const gridData = personas.map((persona) => ({
    ...persona,
    actions: (
      <ButtonGroup size="sm" aria-label={t("postgres:rowActions")}>
  ...
  return (
    <>
      <title>{t("postgres:personasTitle")}</title>
      <Container fluid className="app-page">
        <div className="page-heading page-heading-actions">
          <div>
            <h1>{t("postgres:personasTitle")}</h1>
            <p>{t("postgres:personasSubtitle")}</p>
          </div>
        ...
        <Row>
          <Col>
            {loading ? (
              ...
              <DataGrid data={gridData} schemaColumns="postgres" hiddenColumns={["fechaRegistro"]} pagination filtering />
            ) : (
              <Alert variant="secondary">{t("postgres:emptyResult")}</Alert>
            )}
          </Col>
        </Row>
        ...
        <DynamicModalForm
          ...
          title={modalMode === "create" ? t("postgres:addPersona") : t("postgres:updatePersona")}
...
const getPersonaFields = (t, mode) => [
  { name: "idPersona", type: "number", label: t("postgres:idPersona"), hidden: mode === "create", disabled: true },
  { name: "nombre", type: "text", label: t("postgres:nombre") },
  { name: "apellido", type: "text", label: t("postgres:apellido") },
  { name: "edad", type: "number", label: t("postgres:edad") },
  { name: "email", type: "email", label: t("postgres:email") },
  { name: "fechaNacimiento", type: "date", label: t("postgres:fechaNacimiento") },
  { name: "activo", type: "checkbox", label: t("postgres:activo") }
];
...
export default PostgresPersonasPage;