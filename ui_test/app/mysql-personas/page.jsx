...
import { deleteMysqlPersona, getMysqlPersonas, insertMysqlPersona, updateMysqlPersona } from "../actions/mysqlService";
...
const MysqlPersonasPage = () => {
  ...
  const reloadData = async () => {
    ...
    try {
      const result = await getMysqlPersonas();
      setPersonas(result || []);
    } catch (loadError) {
  ...
  const confirmDelete = async (persona) => {
    ...
    try {
      await deleteMysqlPersona(persona.idPersona);
  ...
  const gridData = personas.map((persona) => ({
    ...persona,
    actions: (
      <ButtonGroup size="sm" aria-label={t("mysql:rowActions")}>
  ...
  return (
    <>
      <title>{t("mysql:personasTitle")}</title>
      <Container fluid className="app-page">
        <div className="page-heading page-heading-actions">
          <div>
            <h1>{t("mysql:personasTitle")}</h1>
            <p>{t("mysql:personasSubtitle")}</p>
          </div>
        ...
        <Row>
          <Col>
            {loading ? (
              ...
              <DataGrid data={gridData} schemaColumns="mysql" hiddenColumns={["fechaRegistro"]} pagination filtering />
            ) : (
              <Alert variant="secondary">{t("mysql:emptyResult")}</Alert>
            )}
          </Col>
        </Row>
        ...
        <DynamicModalForm
          ...
          title={modalMode === "create" ? t("mysql:addPersona") : t("mysql:updatePersona")}
...
const getPersonaFields = (t, mode) => [
  { name: "idPersona", type: "number", label: t("mysql:idPersona"), hidden: mode === "create", disabled: true },
  { name: "nombre", type: "text", label: t("mysql:nombre") },
  { name: "apellido", type: "text", label: t("mysql:apellido") },
  { name: "edad", type: "number", label: t("mysql:edad") },
  { name: "email", type: "email", label: t("mysql:email") },
  { name: "fechaNacimiento", type: "date", label: t("mysql:fechaNacimiento") },
  { name: "activo", type: "checkbox", label: t("mysql:activo") }
];
...
export default MysqlPersonasPage;
